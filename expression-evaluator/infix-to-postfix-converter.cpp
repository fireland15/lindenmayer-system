...
TokenList InfixToPostfixConverter::operator()(const TokenList& in) const {
	if (in.empty())
		return TokenList();
...
bool InfixToPostfixConverter::TestPrecedence(const Token& t1, const Token& t2) const {
	if (t2.Type() == TokenType::LParen)
		return false;

	auto t1Data = m_operatorMap.find(t1.Operator())->second;
	auto t2Data = m_operatorMap.find(t2.Operator())->second;
